...
export default function ClassPlanner({ isEditMode }: ClassPlannerProps) {
    ...
    return (
        <>
            <div className="flex-1 h-full bg-primary-50 dark:bg-gray-800/40 p-8 overflow-y-auto transition-colors duration-300">
                <div className="max-w-3xl mx-auto">
                    <header className="mb-8 text-center relative">
                        <div className="flex items-center justify-center gap-2">

                            <BalletIcon
                                className={`fill-primary-300 dark:fill-primary-500 size-24 inline-block cursor-pointer hover:fill-primary-400 dark:hover:fill-primary-600 transition-colors ${isLeftBallerinaSpinning ? 'animate-ballerina-spin' : ''}`}
                                onClick={handleLeftBallerinaClick}
                            />
                            <h1 className="text-4xl font-serif text-primary-950 dark:text-primary-200 mb-2 font-bold "
                                onClick={handleTitleClick}>
                                Planificación de Clase
                            </h1>
                            <BalletIcon
                                className={`fill-primary-300 dark:fill-primary-500 scale-x-[-1] size-24 inline-block cursor-pointer hover:fill-primary-400 dark:hover:fill-primary-600 transition-colors ${isRightBallerinaSpinning ? 'animate-ballerina-spin' : ''}`}
                                onClick={handleRightBallerinaClick}
                            />
                        </div>
                        <p className="text-primary-800/80 dark:text-primary-300/80 font-medium mb-4">Arrastra ejercicios aquí para construir tu clase</p>
